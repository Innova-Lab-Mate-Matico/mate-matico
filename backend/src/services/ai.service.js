...
export async function explainTheoryTopic({ moduleId, lessonId, theoryId, question, history }) {
  ...
  if (geminiKey && geminiKey.trim() !== '') {
    try {
      ...
      console.error("Error al conectar con Gemini API (SDK):", err);
    }
  }
  ...
  if (openaiKey && openaiKey.trim() !== '') {
    try {
      ...
      console.error("Error al conectar con OpenAI API:", err);
    }
  }

  // 4. MOTOR LOCAL ADAPTATIVO
